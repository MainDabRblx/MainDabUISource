...
namespace MainDabUpdater
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Beta UPDATER : Starting update...");
            Console.Title = "Beta MainDab | Updating MainDab...";
            Thread.Sleep(1000);
            Console.WriteLine("UPDATER : Deleting MainDab...");
            string path = "MainDabBeta.exe";
            if (File.Exists(path))
            {
                File.Delete(path);
            }
            ...
            webClient.DownloadFile("https://github.com/leonardssy/ProjectDab/blob/master/MainDabBeta.exe?raw=true", "MainDabBeta.exe");
            Console.WriteLine("UPDATER : New MainDab downloaded.");
            Console.WriteLine("NOTE : THIS IS A BETA VERSION OF MAINDAB.");
